...
- رقم الهاتف: ${orderData.customer.phone}
- المدينة: ${orderData.customer.city}
- العنوان: ${orderData.customer.address}
- ملاحظات: ${orderData.customer.notes}

المنتجات المطلوبة:
${orderData.items.map(item => `- ${item.name} × ${item.quantity} = ${item.price * item.quantity} د.ل`).join('\n')}

الإجمالي: ${orderData.total} د.ل

التاريخ والوقت: ${orderData.timestamp}
    `.trim();
    ...
    showNotification('تم فتح تطبيق البريد الإلكتروني! يرجى إرسال الرسالة.');
